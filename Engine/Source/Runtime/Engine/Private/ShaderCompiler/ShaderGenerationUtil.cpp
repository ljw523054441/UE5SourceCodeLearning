...
void FShaderCompileUtilities::ApplyFetchEnvironment(FShaderMaterialPropertyDefines& SrcDefines, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	// [caryjwli START] [Add Custom Shading Model C++ PART 4] add macro
	// 该操作是判断当前shading model是否被击中，选择就会再次设为1，没被选中的就会设为0。
	FETCH_COMPILE_BOOL(MATERIAL_SHADINGMODEL_MY_CUSTOM_SHADING_MODEL);
	// [caryjwli END] [Add Custom Shading Model C++ PART 4] add macro
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	// [caryjwli START] [Add Custom Shading Model C++ PART 4] add custom shading model macro
	if (Mat.MATERIAL_SHADINGMODEL_MY_CUSTOM_SHADING_MODEL)
	{
		// 此操作是为了写入GBuffer，不写这段就无法在Shader正常获取GBuffer
		SetStandardGBufferSlots(Slots,bWriteEmissive,bHasTangent,bHasVelocity,bHasStaticLighting,bIsStrataMaterial);
		// 是否往GBuffer写入CustomData
		Slots[GBS_CustomData] = bUseCustomData;
	}
	// [caryjwli END] [Add Custom Shading Model C++ PART 4] add custom shading model macro
