	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	Dst.WRITES_CUSTOMDATA_TO_GBUFFER = (Dst.USES_GBUFFER && (
		// [caryjwli START] [Add Custom Shading Model C++ PART 5] add my custom shading model macro
		// （如果需要使用CustomData的话就要加）这一块区域本身在UE4里是定义在BasePassCommon.ush里的，
		// 到的UE5就移动到C++里面了，使用FETCH进行动态判断宏是否需要开启。
		// 可以看到，在ush里，这个宏虽然有被声明，但是底下的实现被注释掉了，证实了已经不需要再在shader里添加了。
		Mat.MATERIAL_SHADINGMODEL_MY_CUSTOM_SHADING_MODEL ||
		// [caryjwli END] [Add Custom Shading Model C++ PART 5] add my custom shading model macro
		Mat.MATERIAL_SHADINGMODEL_SUBSURFACE || Mat.MATERIAL_SHADINGMODEL_PREINTEGRATED_SKIN || Mat.MATERIAL_SHADINGMODEL_SUBSURFACE_PROFILE || Mat.MATERIAL_SHADINGMODEL_CLEAR_COAT || Mat.MATERIAL_SHADINGMODEL_TWOSIDED_FOLIAGE || Mat.MATERIAL_SHADINGMODEL_HAIR || Mat.MATERIAL_SHADINGMODEL_CLOTH || Mat.MATERIAL_SHADINGMODEL_EYE));
